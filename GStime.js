(function(global) {
     ...
     * @constructor
     * @param {string|Element} selector - The selector or element.
     */
    ...
    GStime.prototype.animate = function(properties, duration, callback) {
        ...
        const initialStyles = this.elements.map(el => 
            Object.fromEntries(Object.keys(properties).map(prop => 
                [prop, parseFloat(getComputedStyle(el)[prop]) || 0]
            ))
        );
    ...
    GStime.prototype.fadeIn = function(duration, callback) {
        return this.show().css('opacity', 0).animate({ opacity: 1 }, duration, callback);
    };
    ...
    GStime.ajax = function(settings) {
        if (!settings || typeof settings.url === 'undefined') {
            return Promise.reject(new Error('A URL is required for the ajax request'));
        }
