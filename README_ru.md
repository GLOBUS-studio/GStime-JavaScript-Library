...
> **v2.0.0** распространяется в формате UMD (глобальная переменная в браузере, CommonJS, AMD). Список ломающих изменений см. в [CHANGELOG.md](CHANGELOG.md).
...
```js
// Глобально в браузере (после <script>)
$('selector')
...
Методы `html()`, `append()`, `prepend()`, `before()`, `after()` и HTML-строки в конструкторе используют `innerHTML`/`insertAdjacentHTML`. **Никогда не передавайте в них непроверенный пользовательский ввод.** Предварительно очищайте данные (например, через DOMPurify) или используйте текст: `.elements[0].textContent = value`.
